...
Restaurant Collection Program --- Choose one
 ...
 d:  Search the collection for cuisine and average price
 t:  Search the collection for phrase
...
def handle_commands(C: list) -> list:
    ...
    while True:
        ...
        if response=="q":
            ...
            n = input("Please enter a phrase to search for: ")
            print(Collection_str(Collection_search_for_phrase(C,n)))
        elif response=='e':
...
def Restaurant_contains_phrase(R: Restaurant, s:str)->bool:
    '''This function returns true if any dish in the restaurants menu has the phrase s'''
    for i in R.menu:
        if s in i.name:
            return True
    return False
...
def Collection_search_for_phrase(C: list, s:str)->list:
    '''Returns list with restaurants with dishes that contain phraase s'''
    temp = []
    for i in C:
        if Restaurant_contains_phrase(i,s):
            temp.append(i)
    return temp
restaurants()
